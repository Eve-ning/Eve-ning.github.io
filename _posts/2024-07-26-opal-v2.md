...
For example, I could describe my abilities with numbers:
...
As shown, we **can** represent a non-numerical player numerically!
But what's more interesting, is when you also place maps in the same chart
...
By intuition, if a map's statistics are mostly smaller than mine, then I'll do well,
see **Map A**. Consequently, if the map's statistics are larger, then I'll do worse,
see **Map B**.
